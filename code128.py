...
#!/usr/bin/env python
# This module is used to generate Code 128 barcodes.
...
# The Code 128 chart is used to map characters to their corresponding barcode patterns.
...
class Code128:
    """
    This class is used to generate Code 128 barcodes.
    """
    ...
    def code128_format(data: str) -> List[int]:
        ...
        This method encodes the input data into a list of integers representing
        the Code 128 barcode. It starts with a start code, followed by the data,
        a checksum, and a stop code. It switches between Code B and Code C
        character sets for optimal encoding.

        Args:
            data (str): The text to be encoded into a Code 128 barcode.

        Returns:
            List[int]: A list of integers representing the encoded barcode.
        """
        text = str(data)
        pos = 0
        length = len(text)
    ...
    def code128_image(
        ...
        """
        Generate a barcode image from the provided data.

        Args:
            data (str): The data to encode in the barcode.
            bar_text (str, optional): The text to display below the barcode. Defaults to None.
            height (int, optional): The height of the barcode. Defaults to 25.
            thickness (int, optional): The thickness of the barcode lines. Defaults to 1.
            quiet_zone (bool, optional): Whether to include a quiet zone in the barcode. Defaults to True.

        Returns:
            str: The path to the generated barcode image.
        """
        og_data = data
        ...
        if bar_text is not None:
            text_img_height = height + 30
            W = width

        # Monochrome Image
        img = Image.new(mode="1", size=(width, text_img_height), color=1)
        ...
        if bar_text is not None:
            # font = ImageFont.truetype("/Users/delio/Library/Fonts/arial.ttf", 16)
            # get dimensions to center text
            w, h = draw.textsize(bar_text)
            draw.text(((W - w) / 2, height + 5), bar_text, fill=0)

        # save the image into a file
        image_dir = f"barcodes/{og_data}.png"

        # Create the directory if it doesn't exist
        makedirs(path.dirname(image_dir), exist_ok=True)
